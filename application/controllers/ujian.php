...
class Ujian extends CI_Controller
{
	...
	public function beranda($offset = 0)
    {
        ...
        $this->data['form_action'] = 'ujian/insert_aktif';
		$ujian = $this->ujian->option_ujian();
        foreach($ujian as $row)
        {
            $this->data['option_ujian'][$row->id_ujian] = $row->mapel." ->> KD (".$row->kd.")";
        }
		$aktif = $this->ujian->ujian_aktif();
		foreach($aktif as $row)
        {
            $this->data['id_ujian'] = $row->id_ujian;
			$this->data['mapel_ujian_aktif'] = $row->mapel;
			$this->data['kd_ujian_aktif'] = $row->kd;
			$this->data['waktu_ujian_aktif'] = ($row->waktu)/60;
        }
		$t_aktif = $this->t_ajaran->t_ajaran_aktif();
		foreach($t_aktif as $row)
        {
            $this->data['t_ajaran'] = $row->t_ajaran;
...
public function insert_aktif()
    {
                    //update db
                    $this->ujian->insert_aktif();
                    $this->session->set_flashdata('pesan', 'Aktifasi Ujian Berhasil.');
                    redirect('ujian/ujian');
					echo $this->input->post('set_ujian');
                }
	public function anu()
	{
		echo  strtotime('minutes','30000');
	}
}
